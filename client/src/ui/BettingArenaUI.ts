...
export class BettingArenaUI {
  ...
  private async handleAuthChange(user: { id: string } | null) {
    if (user && this.battle) {
      try {
        ...
        const response = await fetch(import.meta.env.VITE_BACKEND_URL + `/api/battles/${this.battle.id}/bets?userId=${user.id}`);
        if (response.ok) {
  ...
  private async handleConfirmBet(): Promise<void> {
    ...
    const endpoint = this.battle.bettingType === 'PARIMUTUEL' ? import.meta.env.VITE_BACKEND_URL + `/api/mvp/battles/${this.battle.id}/bet` : `/api/battles/${this.battle.id}/bet`;
    ...
    try {
        ...
        if (response.ok) {
            ...
            const userResponse = await fetch(import.meta.env.VITE_BACKEND_URL + '/api/users/test-user');
            if (userResponse.ok) {
  ...
  private async updateOdds() { // This method will now handle both Odds (AMM) and Pools (Parimutuel)
    ...
    if (this.battle.bettingType === 'PARIMUTUEL') {
        try {
            const response = await fetch(import.meta.env.VITE_BACKEND_URL + `/api/mvp/battles/${this.battle.id}/pools`);
    ...
    // AMM Logic
    try {
      const response = await fetch(import.meta.env.VITE_BACKEND_URL + `/api/battles/${this.battle.id}/odds`);
